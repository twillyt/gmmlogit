...
%   
%   Z: J x K "instrument" matrix (none used if null)
%   w: weighting code (0 = none, 1 = inv( Z' Z ), 2 = generic)
%   W: K x K symmetric positive definite weighting matrix (if w = 2 only)
%   
...
function [bta,flag,code] ...
                = KTRGMMLogit(J,N,K,M,Ms,Y,s,og,Z,w,W,bta0,options)
    ...
    if( isempty(Z) ), % no instruments
        [bta,flag,out] ...
            = GMMLogit_noZ(J,N,K,M,Msa,Y,s,og,w,W,bta0,options);
    else,  % using instruments
        switch( w ),
            % inv( Z' Z ) weighting
            case 1, 
                [bta,flag,out] ...
                    = GMMLogit_w1Z(J,N,K,M,Msa,Y,s,og,Z,bta0,options);
            % generic weighting
            case 2, 
                [bta,flag,out] ...
                    = GMMLogit_w2Z(J,N,K,M,Msa,Y,s,og,Z,W,bta0,options);
            % no weighting (identity)
            otherwise,
                [bta,flag,out] ...
                    = GMMLogit_w0Z(J,N,K,M,Msa,Y,s,og,Z,bta0,options);
        end
...
function [bta,flag,out] ...
    ...
    % solve
    [x,fval,flag,out] = ktrlink(obj,x0,[],[],Aeq,beq,[],[],nlc,options);
...
%       min x' W x / 2
%       wrt beta, x
%       sto log( P(beta,x) ) - log( s ) = 0 (nonlinear equality constraint)
% 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [bta,flag,out] ...
    ...
    % initial condition
    x0 = [ bta0 ; randn(J,1) ]; % beta, then xsi
    ...
    % solve
    [x,fval,flag,out] = ktrlink(obj,x0,[],[],[],[],[],[],nlc,options);
